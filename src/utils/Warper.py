...
class Warper:
    ...
    def bilinear_splatting(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: Optional[torch.Tensor],
        ...
        if mask1 is None:
            mask1 = torch.ones(size=(b, 1, h, w)).to(frame1)
        if depth1 is None:
            depth1 = torch.ones(size=(b, 1, h, w)).to(frame1)
