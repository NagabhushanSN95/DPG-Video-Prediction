...
# Extended from SintelTester01.py. Dataloaders are used to load the data
...
class DpgVideoPredictor:
    ...
    def load_data(self, video_name, seq_name, pred_frame_num):
        input_data = self.data_loader.load_test_data(video_name, seq_name, pred_frame_num)
        return input_data
    ...
    def predict_next_frame2(self, video_name: str, seq_name: str, pred_frame_nos: List[int]) -> List[numpy.ndarray]:
        """
        ...
        for pred_frame_num in pred_frame_nos:
            input_dict = self.load_data(video_name, seq_name, pred_frame_num)
            next_frame = self.predict_next_frame1(input_dict)
            frames[pred_frame_num] = next_frame

        gt_frame_nos = sorted(set(range(self.num_frames_data[video_name])) - set(pred_frame_nos))
        for gt_frame_num in gt_frame_nos:
            gt_frame_path = self.database_dirpath / f'all/RenderedData/{video_name}/rgb/{seq_name}/{gt_frame_num:04}.png'
    ...
    def post_process_output(output_batch: dict):
        ...
        for key in output_batch.keys():
            if isinstance(output_batch[key], torch.Tensor):
                processed_batch[key] = numpy.moveaxis(output_batch[key].detach().cpu().numpy(), [0, 1, 2, 3], [0, 3, 1, 2])[0, 6:-6]
...
def start_testing(test_configs: dict):
    ...
    videos_data = frames_data[['video_name', 'seq_name']].drop_duplicates().to_numpy()
    ...
    for video_data in tqdm(videos_data, total=num_videos):
        ...
        num_frames = video_predictor.num_frames_data[video_name]
        pred_frame_nos = list(range(7, num_frames, 2))
        ...
        if not (pred_frames_dirpath / f'{last_pred_frame_num:04}.png').exists():
            all_frames = video_predictor.predict_next_frame2(video_name, seq_name, pred_frame_nos)
    ...
    # Run QA
    qa_filepath = Path('../../../../QA/00_Common/src/AllMetrics02_Sintel.py')
    cmd = f'python {qa_filepath.absolute().as_posix()} ' \
          ...
          f'--pred_masks_dirname VSR001_101_1step_forward '
    os.system(cmd)
    return
...
def demo2():
    ...
    configs = {
        ...
        'database_dirpath': 'MPI_Sintel/Data',
        'device': 'gpu0',
    }
