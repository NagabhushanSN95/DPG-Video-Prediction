...
# Extended from OursBlenderTester01.py for multi-step prediction
...
class DpgVideoPredictor:
    def __init__(self, root_dirpath: Path, database_dirpath: Path, train_configs: dict, device: str = 'gpu0', short_videos: bool = True):
    ...
    def load_data(self, video_name, seq_num, pred_frame_nos):
        ...
        for frame_num in gt_frame_nos:
            frame_path = self.database_dirpath / f'{self.all_dirname}/RenderedData/{video_name}/seq{seq_num:02}/rgb/{frame_num:04}.png'
...
def get_all_pred_frame_nums(frames_data: pandas.DataFrame):
    pred_frame_nums = sum([literal_eval(x) for x in numpy.unique(frames_data['pred_frame_num'])], [])
    pred_frame_nums = sorted(pred_frame_nums)
    return pred_frame_nums
...
def start_testing(test_configs: dict):
    ...
    # Restore DPG Model
    short_videos = test_configs['videos_duration'] == 'short'
    video_predictor = DpgVideoPredictor(root_dirpath, database_dirpath, train_configs, test_configs['device'], short_videos)
    ...
    pred_frame_nos = get_all_pred_frame_nums(frames_data)
