...
# Computes both perceptual and style loss together
...
class PerceptualAndStyleLoss(LossFunctionParent):
    ...
    def compute_loss(self, input_dict: dict, output_dict: dict):
        ...
        known_perceptual_loss, known_style_loss = self.compute_perceptual_and_style_loss(target_frame_features, predicted_frame_features, occlusion_map)
        unknown_perceptual_loss, unknown_style_loss = self.compute_perceptual_and_style_loss(target_frame_features, predicted_frame_features, 1 - occlusion_map)
        ...
        total_loss = self.perceptual_loss_weight * perceptual_loss + self.style_loss_weight * style_loss
        return total_loss

    def compute_perceptual_and_style_loss(self, target_frame_features: torch.Tensor,
                                          predicted_frame_features: torch.Tensor, mask: torch.Tensor):
