...
class OursBlenderDataLoader(DataLoaderParent):
    """
    ...
    def __init__(self, configs: dict, data_dirpath: Path, frames_datapath: Optional[Path]):
        super(OursBlenderDataLoader, self).__init__()
    ...
    def create_samples(self, frames_datapath: Path):
        ...
        video_seqs = video_data[['video_name', 'seq_num']].drop_duplicates()
        samples_data = []
        for i, (video_name, seq_num) in video_seqs.iterrows():
            for pred_frame_num in range(self.num_past_frames, self.num_frames_per_seq):
                samples_data.append([video_name, seq_num, pred_frame_num])
        return samples_data

    def create_memmaps(self):
        print('Memory mapping the database')
        for video_name, seq_num, pred_frame_num in tqdm(self.samples_data):
            if video_name not in self.frames.keys():
                self.frames[video_name] = {}
            if seq_num not in self.frames[video_name].keys():
                self.frames[video_name][seq_num] = {}

            for frame_num in range(pred_frame_num - 4, pred_frame_num + 1):
                if frame_num in self.frames[video_name][seq_num].keys():
                    continue
                frame_path = self.dataroot / f'all_short/RenderedData/{video_name}/seq{seq_num:02}/rgb/{frame_num:04}.npy'
                self.frames[video_name][seq_num][frame_num] = self.read_npy_file(frame_path, mmap_mode='r')
        return
    ...
    def load_training_data(self, index: int, random_crop: bool = True, random_flip: bool = True):
        video_name, seq_num, pred_frame_num = self.samples_data[index]
        ...
        for frame_num in range(pred_frame_num - 4, pred_frame_num):
            frame = self.frames[video_name][seq_num][frame_num]
        ...
        target_frame = self.frames[video_name][seq_num][pred_frame_num]
    ...
    def load_testing_data(self, video_name: str, seq_num: int, pred_frame_num: int, padding: tuple = None):
        ...
        past_frames = []
        for frame_num in range(pred_frame_num - 4, pred_frame_num):
            frame_path = self.dataroot / f'all_short/RenderedData/{video_name}/seq{seq_num:02}/rgb/{frame_num:04}.png'
